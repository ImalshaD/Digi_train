...
from keras.layers import MultiHeadAttention, Dense, Input
from keras.models import Model

def trans_model(input_dim, output_dim, activation="leaky_relu", dropout=0.2):
    ...
    # Replace LSTM with Transformer Encoder Layer
    attention_output = MultiHeadAttention(num_heads=8,
                                          key_dim=x9.shape[-1] // 8,
                                          name='encoder_attention')(squeezed,
                                                                     squeezed)
    attention_output = layers.Dropout(dropout)(attention_output)
    attention_output = layers.LayerNormalization(epsilon=1e-6)(squeezed + attention_output)

    output = Dense(output_dim + 1,
                   activation="softmax",
